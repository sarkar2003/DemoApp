...
''');
  ...
  Future insert(data, imagebits, liked) async {
    ...
    var response = await db
        .insert("LIKE", {"DATA": data, "IMAGE": imagebits, "ISLIKED": liked});
  ...
  Future update(id, data, imagebits, liked) async {
    ...
    var response = await db.update(
        "LIKE", {"DATA": data, "IMAGE": imagebits, "ISLIKED": liked},
        where: 'id=?', whereArgs: [id]);

    var response2 = await db.rawQuery('SELECT * FROM LIKE');
    print(response2);
    print("i am here");
  ...
  Future readdata() async {
    ...
    var response = await db.rawQuery('SELECT * FROM LIKE');
  ...
  Future deletedata() async {
    Database db = await database;
    return await db.rawQuery('DELETE FROM LIKE');
  }
}
